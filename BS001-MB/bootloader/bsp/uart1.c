...
#define UART1_BUFFER_SIZE_TRANSMIT 256
#define UART1_BUFFER_SIZE_RECEIVE 256
...
int32_t uart1_transmitIdleBufferSize(void)
{
	if(uart.tail_transmit <= uart.head_transmit)
		return UART1_BUFFER_SIZE_TRANSMIT - uart.head_transmit + uart.tail_transmit;
...
int32_t uart1_transmitValidBufferSize(void)
{
	...
	return uart.head_transmit + (UART1_BUFFER_SIZE_TRANSMIT - uart.tail_transmit);
}
...
void uart1_transmitAByte(void)
{
	...
	uart.tail_transmit ++;
	if(uart.tail_transmit >= UART1_BUFFER_SIZE_TRANSMIT)
		uart.tail_transmit = 0;
}
...
uint8_t uart1_transmit(const void * data,uint32_t len)
{
	...
	for(int i=0;i<len;i++)
	{
		...
		//
		uart.head_transmit ++;
		if(uart.head_transmit >= UART1_BUFFER_SIZE_TRANSMIT)
			uart.head_transmit = 0;
		//
	}
	//
...
int32_t uart1_receiveValidBufferSize(void)
{
	...
	return uart.head_receive + (UART1_BUFFER_SIZE_RECEIVE - uart.tail_receive);
}
...
int32_t uart1_readReceiveBuffer(void * outputBuffer,int32_t outputBufferLength)
{
    ...
    for(uint32_t i=0;i<returnLength;i++)
    {
        ...
        //
        uart.tail_receive ++;
        if(uart.tail_receive == UART1_BUFFER_SIZE_RECEIVE)
            uart.tail_receive = 0;
    }
    //
    return returnLength;
